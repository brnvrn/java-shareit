...
public class BookingService {
    ...
    public BookingDto addNewBooking(Long userId, BookingRequestDto bookingRequestDto) {
        ...
        if (startBooking == null || endBooking == null) {
            throw new java.lang.IllegalArgumentException("Время начала и окончания бронирования не может быть null");
        }
    ...
    public BookingDto approvedOrRejectBooking(Long userId, Long bookingId, Boolean approved) {
        ...
        if (!booking.getItem().getOwner().getId().equals(userId)) {
            throw new IllegalArgumentException("Вы не являетесь собственником вещи");
        }
    ...
    public List<BookingDto> getAllBookingByOwner(Long userId, BookingState state) {
        ...
        if (bookings.isEmpty()) {
            throw new IllegalArgumentException("У вас нет бронирований для просмотра");
        }
        return switch (state) {
